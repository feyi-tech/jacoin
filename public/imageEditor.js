class ImageEditor {
    constructor() {
        this.editorClassname = "app-explainer-image-editor"
    ...
    activatePenTool() {
        ...
        let lastClickTime = 0;
        let lastPoint = null; // Stores the first point for straight lines
        ...
        // Double-Click to Draw a Straight Line
        drawPane.addEventListener("dblclick", (e) => {
            console.log("dblclick: ", e)
            const currentTime = new Date().getTime();
            const clickPosition = { x: e.offsetX, y: e.offsetY };
    
            if (lastClickTime && currentTime - lastClickTime < 2000) {
                // If a second double-click happens within the time frame, draw a straight line
                if (lastPoint) {
                    ctx.strokeStyle = strokeColor;
                    ctx.lineWidth = strokeWidth;
                    ctx.beginPath();
                    ...
                    ctx.stroke();
                    ctx.closePath();
                    lastPoint = null; // Reset for next straight line
                    lastClickTime = 0;
                }
            } else {
                lastPoint = clickPosition; // Store first click position
    ...
    getGlobalPosition(element) {
        if (!element) return {x: 0, y: 0};
    
        let x = 0, y = 0;
        let currentElement = element;
    
        // Traverse up the DOM tree to accumulate offsets
    ...
    commitToCanvas(box, content, type) {
        ...
        // Convert absolute positions to relative canvas coordinates
        //var x = rect.left - canvasRect.left;
        //var y = rect.top - canvasRect.top;
        const { x, y } = this.getGlobalPosition(typeof content != "string" && type != "image"? content : box)
        let width, height;
        ...
        const computedStyles = window.getComputedStyle(typeof content != "string"? content : box);
        let fontSize = parseInt(computedStyles.fontSize, 10);
        let fontHeight = this.removeUnit(fontSize) * 1.2;
    
        if (type === "text") {
            // Get computed styles for text
            width = rect.width;
            height = fontSize * 1.3; // Approximate text height
        } else {
            width = rect.width;
            height = rect.height;
        }
        ...
        // Translate to the center of the box, ensuring correct positioning
        ...
        if (type === "text") {
            ...
            if (rotation !== 0) {
                ...
                const centerY = y //+ (rect.height / 2)// + height / 2;
            ...
            const padding = 0//8; // Text padding
            ...
            // Word wrapping logic
            const wrappedLines = []//["abcd", "efghi", "ijklno"];
            let maxText = this.ctx.measureText("ijklno")
            const maxTextWidth = maxText.actualBoundingBoxRight - maxText.actualBoundingBoxLeft;
            console.log("True Text Width:", maxTextWidth);
            const words = textContent.split(" ");
            let currentLine = "";
            for (let word of words) {
                let testLine = currentLine + (currentLine ? " " : "") + word;
                let test = this.ctx.measureText(testLine);
                let testWidth = test.actualBoundingBoxRight - test.actualBoundingBoxLeft;
                console.log("wrappedLines.word", testLine, testWidth, width, this.ctx.measureText(testLine))
            ...
            let yOffset = lineHeight + 6;
            for (let i = 0; i < wrappedLines.length; i++) {
                const line = wrappedLines[i]
                let testWidth = this.ctx.measureText(line).width
                let textX = x + ((maxTextWidth / 2) - (testWidth / 2)); // Center align text horizontally
                let textY = y + lineHeight + (yOffset * i); // Center align text horizontally
                if (textAlign === "left") textX = x;
                if (textAlign === "right") textX = x + (maxTextWidth - testWidth);

                console.log("Object drawn at:3", textX, textY, textAlign)
                this.ctx.fillText(line, textX, textY);
            }*/

            console.log("Object drawn at:2", fontSize, canvasRect.left + (rect.width / 2), canvasRect.top + (rect.height / 2), x, y);
            this.ctx.fillText(textContent, x, y + (fontHeight / 2))
        } else if (type === "emoji") {
            // Ensure emoji is centered properly
            //const fontSize = parseInt(box.style.fontSize || "0", 10);
            ...
            // Draw emoji exactly at the center of the box
            //this.ctx.fillText(content, width / 2, height / 2);
            this.ctx.fillText(content, x + fontSize / 2, y + fontSize / 2);
        } else {
            // Handle image rendering
            const img = new Image();
            img.src = content.src;
            img.onload = () => {
                // Ensure image is drawn perfectly centered
        ...
        // Remove the selection box after committing
        box.remove();
        this.removeExistingUI();
    }    

    commitToCanvas2(box, content, type) {
        const rect = box.getBoundingClientRect();
        const canvasRect = this.canvas.getBoundingClientRect();
        ...
        if (type === "text") {
            ...
            width = rect.width;
            height = fontSize * 1.3; // Estimate text height
        } else {
            width = rect.width;
            height = rect.height;
        }
        ...
        this.ctx.save();
        this.ctx.translate(x + width / 2, y + height / 2); // Move to the center
        this.ctx.rotate((rotation * Math.PI) / 180);
        this.ctx.translate(-width / 2, -height / 2); // Move back

        console.log("computedStyles", rect.width, rect.height, rect.left, rect.top, content, box)
        ...
        if (type === "text") {
            ...
            const fontSize = Number((box.style.fontSize || "0").replace("px", ""))
            this.ctx.font = `${fontSize}px Arial`;
            this.ctx.textAlign = "center";
            //this.ctx.fillText(content, width / 2, height);
            console.log("box.style.left, box.style.top", fontSize / 2, fontSize, "fs", fontSize)
            console.log("x, y", x, y)
            this.ctx.fillText(content, fontSize / 2, fontSize);
            //this.ctx.fillText(content, 50, 0);
